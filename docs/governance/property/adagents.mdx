...
Implementations that expose validator results SHOULD provide structured provenance metadata (rather than only warning text), e.g.:

- `discovery_method: "direct" | "authoritative_location" | "ads_txt_managerdomain"`
- `manager_domain` when `discovery_method === "ads_txt_managerdomain"`
